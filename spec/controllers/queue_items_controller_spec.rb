...
describe QueueItemsController do
  ...
  describe "GET index" do
    context "with authenticated user" do
      ...
      let(:queued_item1) { object_generator(:queue_item, list_position: 2, user: current_user) }
      let(:queued_item2) { object_generator(:queue_item, list_position: 1, user: current_user) }
    ...
    context "with authenticated user" do
      ...
      context "when the item is not already in the list" do
        ...
        it "creates a new queue item with the assocaited user" do
          post :create, video_id: new_video.id, user_id: current_user.id
        ...
        it "puts the queued item last in the list of queued items" do
          ...
          object_generator(:queue_item, video: exisiting_video, user: current_user)
          
          post :create, video_id: new_video.id, user_id: current_user.id
          find_new_video = QueueItem.where(video_id: new_video.id, user_id: current_user.id).first
      ...
      context "when the item is already in the list" do
        ...
        before do
          set_current_user_session
          object_generator(:queue_item, video: new_video, user: current_user)
          post :create, video_id: new_video.id, user_id: current_user.id
        end
  ...
  describe "DELETE destroy" do
    context "with authenticated user" do
      ...
      context "with queued list associated with authenticated user" do
        let(:new_video) { object_generator(:video) }
        let!(:queue_item) { object_generator(:queue_item, video: new_video, user: current_user) }
        ...
        it "normalizes the queue items" do
          ...
          queue_item2 = object_generator(:queue_item, video: new_video, user: current_user, list_position: 2)
          delete :destroy, id: queue_item.id
